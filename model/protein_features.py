...
class PositionalEncodings(nn.Module):
    ...
    def forward(self, E_idx):
        ...
        ii = torch.arange(N_nodes, dtype=torch.float32).view((1, -1, 1)) #.to(torch.device("mps"))
...
class ProteinFeatures(nn.Module):
    ...
    def _rbf(self, D):
        ...
        D_mu = torch.linspace(D_min, D_max, D_count) #.to(torch.device("mps"))
    ...
    def forward(self, X, L, mask):
        ...
        elif self.features_type == "justas":
            V = torch.zeros_like(AD_features)
            b = X[:,:,1,:] - X[:,:,0,:]
            c = X[:,:,2,:] - X[:,:,1,:]
